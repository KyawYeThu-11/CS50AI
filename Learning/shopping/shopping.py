        ...
        for row in reader:
            ...
            for column in row:
                ...
                if column != 'Revenue':
                    ...
                    value = int(float(value)) if round(float(value)) == float(value) else float(value) 
                    user_evidence.append(value)
                else:
                    value = 1 if value == 'TRUE' else 0
                    labels.append(value)
...
def evaluate(labels, predictions):
    ...
    for i, label in enumerate(labels):
        ...
        elif label == 0 and label == predictions[i]:
            correct_negative_labels += 1
    
    sensitivity = correct_positive_labels / labels.count(1)
    specificity = correct_negative_labels / labels.count(0)
